...
The first metadata sets are Clang, GCC, GNU ld, LLD, MSVC, mold, and NVCC
options. See [`option`](option) for generated TOML files, provenance, and
reproducible pixi-managed regeneration and verification commands. See
[`docs/option-sources.md`](docs/option-sources.md) for extraction strategies for
additional tools and future probe-based enrichment.
